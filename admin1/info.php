...
<script>
	...
	function GetAllData(){  
		$.ajax({  
			url: "lib/library.php?action=get_info",
			type: "GET",
	...
	function updateform(){  //alert('hii');
		$("#updateform").submit(function(event) { 
			...
			event.stopImmediatePropagation();
			$.ajax({
				url: 'lib/library.php?action=update_info',
				type:'POST',
				data: new FormData(this),
				cache:false,
				contentType:false,
	...
	function View(data){ //alert(data);
		$.ajax({
			type:"POST",
			url:"lib/library.php?action=veiw_info",
			data:{viewUser:data},
	...
	function deleteUser(data){
		...
		if(agree){
			$.ajax({
				type:"POST",
				url:"lib/library.php?action=deleteinfo",
		...
		<div class="portlet box grey-cascade">
			<div class="portlet-title">
				<div class="caption"style="text-transform: uppercase;font-weight: bold;">
					<i class="fa fa-globe"></i>Info List
				</div>
				<div class="tools">
									...
									<div class="form-body">
										<div class="row">
											...
											<div class="col-md-12">
												<p>Address</p>
												<div class="form-group">
													<input type="textarea" name="address" id="address" class="form-control" placeholder="Address">
												</div>
											</div>
										</div>
									</div>
